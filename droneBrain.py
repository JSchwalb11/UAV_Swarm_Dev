...
class Drone:
    ...
    def __init__(self, config):
        ...
        # logger config
        self.dronelog.logger = logging.getLogger("drone" + (str)(self.id) + "_log")
        self.dronelog.logger.setLevel(logging.DEBUG)
    ...
    def location_callback(self):
        ...
        self.dronelog.logger.info("Drone Location Changed: " + str(self.vehicle.location.global_relative_frame))
    ...
    def armed_callback(self):
        self.dronelog.logger.info("Drone Armed Status Changed: " + str(self.vehicle.armed))
        self.update_self_to_swarm("/Swarm")

    def mode_callback(self):
        self.dronelog.logger.info("Mode Changed: " + str(self.vehicle.mode.name))
        self.update_self_to_swarm("/Swarm")
            ...
            if swarm_params.Drones[idx].id == droneID:
                while (swarm_params.Drones[idx].altitude <= altitude * 0.95):
                    ...
                    print("Waiting for Drone: " + str(swarm_params.Drones[idx].id) + " to reach " + str(
                        altitude))
                    time.sleep(1)
                self.dronelog.logger.info("Drone: " + swarm_params.Drones[idx].id + " reached " + str(altitude) + "...")
